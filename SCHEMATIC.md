...
```
 ...
 │                                                                                              │
 │   pod_00  Mac-H/HARNS  (orchestrator+browser+brain)                                          │
 ...
 │   └── Tailscale host + ACL ........ node "pod_00"                                              │
 │                                                                                              │
...
|---|---|---|---|---|
| pod_00 | OpenHands UI | 3000 | tailnet only | session |
| pod_00 | git (ssh) | 22 | tailnet only | SSH key |
| pod_00 | worker coordinator | 8410 | tailnet only | token |
| pod_02 | worker agent | 8411 | tailnet only | token |
| pod_01 | worker agent | 8412 | tailnet only | token |
...
1. Team member opens `https://pod_00.<tailnet>.ts.net` (OpenHands).
...
4. Coordinator (Mac-H) sees the new ref and dispatches:
   - `pod_02` → xcodebuild on that ref → iOS test result.
   - `pod_01` → gradle/androidTest on that ref → Android test result.
...
- One pod/Mac dies → re-run the **bootstrap script** for its role on a spare identical Mac; the
